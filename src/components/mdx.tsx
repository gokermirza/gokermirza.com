...
// Headings may contain inline markup (bold/italic/links), so children can be a
// React element or array rather than a plain string. Flatten it to text before slugifying.
function nodeToText(node: ReactNode): string {
  if (node == null || typeof node === "boolean") return "";
  if (typeof node === "string" || typeof node === "number") return String(node);
  if (Array.isArray(node)) return node.map(nodeToText).join("");
  if (React.isValidElement(node)) return nodeToText((node.props as { children?: ReactNode }).children);
  return "";
}
